...
public class Main {
      ...
      switch (principal) {
        case 1:// Menu ruleta
          if (Jugador.players.size() == 0) { // Si la lista esta vacia -> out
            System.out.println(
                ConsoleColors.RED + "You need to add a player in the list" + ConsoleColors.RESET);
            break;
          }
          ...
          do {
            System.out.println("Credito: " + aux.getMoney());
  ...
  static int personMenu() {
    Menu person = new Menu("--PERSON MENU--", "Add player", "Erase player", "Print all the players",
        "Change options of the person");
    return person.manage();
  }

  static int changePersonMenu() {
    Menu change = new Menu("--CHANGE PERSON MENU--", "Change dni", "Change name", "Change surnames",
        "Change age", "Change location");
